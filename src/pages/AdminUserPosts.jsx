...
const AdminUserPosts = () => {
    ...
    const fetchPosts = async () => {
        try {
            ...
            toast.error(error.response?.data?.message || "Failed to load user's posts");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSoftDelete = async (e) => {
        ...
        try {
            const response = await softDeletePostAPI(reasonModal.postId, reasonModal.reason);
            toast.success("Post soft deleted successfully");
            
            // Update UI
            setPosts(posts.map(post => post._id === reasonModal.postId ? response.data.data : post));
    ...
    const handleRestore = async (postId) => {
        if (!window.confirm("Are you sure you want to restore this post? It will be visible to the public again.")) return;
    ...
    const handleHardDelete = async (postId) => {
        if (!window.confirm("WARNING: This will permanently delete the post and its image. Continue?")) return;
        ...
        <div className="min-h-screen bg-gray-50 pb-10">
            ...
            <nav className="bg-gray-900 text-white shadow-md px-8 py-4 flex justify-between items-center sticky top-0 z-50">
                <div className="flex items-center gap-4">
                    <button onClick={() => navigate('/admin/dashboard')} className="text-gray-300 hover:text-white font-medium text-lg">
                        ← Back to Dashboard
                    </button>
                    <h1 className="text-xl font-bold border-l border-gray-700 pl-4">Moderation Panel</h1>
                </div>
            </nav>

            <div className="max-w-3xl mx-auto mt-8 px-4">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-bold text-gray-800">User's Content History</h2>
                ...
                <div className="space-y-6">
                    {posts.length === 0 ? (
                        ...
                        posts.map(post => (
                            <div key={post._id} className={`p-6 rounded-2xl shadow-sm border ${post.isSoftDeleted ? 'bg-red-50 border-red-200' : 'bg-white border-gray-100'}`}>
                                
                                {/* Status Badge */}
                                <div className="flex justify-between items-center mb-4">
                                    <span className="text-sm text-gray-500">{new Date(post.createdAt).toLocaleString()}</span>
                                    {post.isSoftDeleted ? (
                                        <span className="bg-red-100 text-red-700 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide">Soft Deleted</span>
                                    ) : (
                                ...
                                {post.imageUrl && (
                                    <img src={post.imageUrl} alt="Post" className="w-full max-h-[400px] object-cover rounded-xl mb-4 border border-gray-200" />
                                )}
                                ...
                                {post.isSoftDeleted && (
                                    <div className="bg-white rounded-lg p-4 mb-4 border border-red-100">
                                        <p className="text-sm text-gray-700 mb-2">
                                            <strong className="text-red-600">Admin Reason:</strong> {post.deletedByReason}
                                        </p>
                                        {post.userClarification ? (
                                            ...
                                            <p className="text-sm text-gray-400 italic">User has not appealed yet.</p>
                                        )}
                                    </div>
                                )}
